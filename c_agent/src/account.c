...
static void on_order(const p2c_order_t *order, void *user)
{
    ...
    /* Enqueue to a take worker. The decision stays here (WS thread) but the
     * blocking 600-1900ms POST is handed off so the WS service loop keeps
     * reading new list:update frames. Firing inline blinded us to every order
     * that arrived during a take — a far worse trade than the ~50us handoff. */
...
/* ---- take thread ----------------------------------------------------- */
/* Consumes the take ring (non-blocking handoff from the WS thread) and runs the
 * blocking POST here, so the WS thread is never stalled. While idle it warms the
 * CURL connection every 8s so the next take reuses a hot TLS/H2 conn. */
